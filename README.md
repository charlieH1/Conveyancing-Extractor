...
- **Query verb** - If the [query verb](https://www.rfc-editor.org/info/rfc10008/) was available in .net I would use it over post when calling the action `Scrape` in `ScraperController` however post was chosen to allow for a longer list of locations and sources in the future
- **Locations** The locations provided in the task were used to create the default locations and are what was tested, in future a longer list of locations may be used or an external address provider could be used to validate/check additional locations would be approved
